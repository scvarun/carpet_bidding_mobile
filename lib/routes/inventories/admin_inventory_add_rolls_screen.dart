...
class AdminInventoryAddRollsScreen extends StatelessWidget {
  static const routeName = '/admin/inventories/addRoll';

  const AdminInventoryAddRollsScreen({Key? key}) : super(key: key);
    ...
    return BlocBuilder<AuthBloc, AuthState>(
      builder: (context, state) {
        if (state is AuthLoggedInState) {
          return const _AdminInventoryAddRollsRender();
        }
        return Container();
      },
    );
...
class _AdminInventoryAddRollsRender extends StatefulWidget {
  const _AdminInventoryAddRollsRender({Key? key}) : super(key: key);

  @override
  State<_AdminInventoryAddRollsRender> createState() =>
      __AdminInventoryAddRollsRenderState();
}

class __AdminInventoryAddRollsRenderState
    extends State<_AdminInventoryAddRollsRender> {
  ...
  Widget build(BuildContext context) {
    return MainLayout(
      ...
      child: FutureBuilder(
        future: Future.wait(
            <Future>[_getCatalogues, _getImporters, _getInventories]),
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.stretch,
                      children: [
                        _patternNoField(context, e),
                        _quantityField(context, e,
                            disableMargin:
                                _patterns[e].type == ApiInventoryTypes.catalog),
                        _similarItems(context, e),
                      ],
                    ),
                  ),
                ],
  ...
  Widget _rateField(BuildContext context) {
    return Container(
      margin: const EdgeInsets.only(bottom: 20, left: 65),
      child: TextFormField(
          enabled: false,
          textInputAction: TextInputAction.next,
  ...
  Widget _similarItems(BuildContext context, int index) {
    return Container(
      ...
      child: DropdownSearch<ApiInventory>.multiSelection(
        ...
        itemAsString: (item) => item!.roll?.patternNo ?? '',
        items: _availableInventories,
        selectedItems: _patterns[index].similarInventories,
        onChanged: (e) {
          setState(() {
            _patterns[index].similarInventories = e;
          });
        },
      ),
    );
  ...
  void _submit(BuildContext context) async {
    ...
    if (_formKey.currentState!.validate()) {
      try {
        ...
        await inventory_repo.addInventories(auth,
            catalogue: _catalogue!, importers: _importers, patterns: _patterns);
        setState(() {
          _success = true;
        });
  ...
  Future<List<ApiInventory>?> _loadInventories(BuildContext context) async {
    ...
    var auth = await refreshToken(context, authState);
    var inventories =
        await inventory_repo.loadInventories(auth, ApiInventoryTypes.rolls);
    setState(() {
      if (inventories != null) {
