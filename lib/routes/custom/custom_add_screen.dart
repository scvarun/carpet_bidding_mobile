...
class CustomAddScreen extends StatelessWidget {
  static const routeName = '/admin/custom/add';

  const CustomAddScreen({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<AuthBloc, AuthState>(
      builder: (context, state) {
        if (state is AuthLoggedInState) return const _CustomAddRender();
        return Container();
      },
    );
  }
...
class _CustomAddRender extends StatefulWidget {
  const _CustomAddRender({Key? key}) : super(key: key);

  @override
  State<_CustomAddRender> createState() => __CustomAddRenderState();
}

class __CustomAddRenderState extends State<_CustomAddRender> {
  ...
  void _submit(BuildContext context) async {
    ...
    if (_formKey.currentState!.validate()) {
      try {
        ...
        await custom_repo.addCustomOrder(auth, _model, _media);
        setState(() {
          _success = true;
        });
